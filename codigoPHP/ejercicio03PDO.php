                ...
                <?php
                        ...
                        if($aErrores['codigoDep']==null){
                            ...
                            if(preg_match($patron, $codigoFormulario)==0){
                                ...
                                try{
                                    ...
                                    $seleccion=$conn->prepare("SELECT T02_CodDepartamento FROM DB208DWESProyectoTema4.T02_Departamento WHERE T02_CodDepartamento LIKE :codigo;");
                    ...
                    if($entradaOK){
                        ...
                        try{
                            $conn = new PDO(SERVIDOR, USUARIO, CONTRASENA);
                            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                            $query=$conn->prepare("INSERT INTO DB208DWESProyectoTema4.T02_Departamento (T02_CodDepartamento, T02_DescDepartamento, T02_VolumenDeNegocio) VALUES (:codigo, :descripcion, :volumen);");
                            $query->bindParam(':codigo', $aRespuestas['codigoDep']);
                            $query->bindParam(':descripcion', $aRespuestas['descripionDep']);
                            $query->bindParam(':volumen', $aRespuestas['volumenNeg']);
                            $query->execute();
            ...
            <div>
                <?php
                    try{
                        $conn = new PDO(SERVIDOR, USUARIO, CONTRASENA);
                        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                        $query=$conn->query("SELECT * FROM T02_Departamento ORDER BY T02_FechaCreacionDepartamento DESC;");
                        if ($query!=null){
                ?>
                <table>
                    <thead>
